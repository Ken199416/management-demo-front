        ...
        >
          ...
          >
            ...
            <el-menu-item
              ...
              @click="clickMenu(subItem.path, item.menuName, subItem.menuName)"
            >
              <template slot="title">
  ...
  methods: {
    ...
    addTab(targetName, contentName) {
      if (
        ...
        // 存在，切换当前的tab
        this.editableTabsValue = this.editableTabs.filter(
          (tab) => tab.title === contentName
        )[0].name;
      }
    },
    removeTab(targetName) {
      console.log("进入removeTab");
    ...
    async getMenu() {
      const response = await this.$http.get("/leftMenu/getAll");
